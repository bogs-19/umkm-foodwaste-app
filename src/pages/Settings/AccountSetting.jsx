...
import { ArrowLeft, User, LogOut, ChevronRight, Check, X, AlertTriangle } from 'lucide-react';
...
const AccountSetting = () => {
    ...
    useEffect(() => {
        ...
        if (savedUser) {
            ...
            const defaultUser = { name: 'Radhitama', email: 'radhitama@umkm.com' };
            setUser(defaultUser);
    ...
    const handleSaveCardProfile = async () => {
        try {
            if (!profileForm.name || !profileForm.email) throw new Error("Nama dan email tidak boleh kosong.");
        ...
        <div className="flex flex-col min-h-full w-full max-w-md mx-auto pb-24 relative overflow-x-hidden px-4 pt-6 text-white">
            ...
            <div className="bg-[#1C1C24] rounded-[28px] shadow-lg border border-white/5 overflow-hidden mb-6">
                <button
                    ...
                    className={`w-full flex items-center justify-between p-5 transition-colors ${isEditingCard ? 'bg-white/5' : 'hover:bg-[#252530]'}`}
                >
            ...
            {errorModal.isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0B1528]/80 backdrop-blur-md animate-fade-in">
                    <div className="bg-[#1C1C24] border border-white/10 w-full max-w-sm rounded-[32px] shadow-2xl p-8 text-center animate-slide-down">
                        ...
                        <p className="text-gray-400 text-sm mb-8 leading-relaxed font-medium">{errorModal.message}</p>
                        <button onClick={() => setErrorModal({ isOpen: false, message: '' })} className="w-full py-4 bg-[#252530] hover:bg-[#30303b] text-white rounded-2xl font-bold transition-colors">Mengerti</button>
                    </div>
                </div>
            )}
            ...
            {logoutModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0B1528]/80 backdrop-blur-md animate-fade-in">
                    <div className="bg-[#1C1C24] border border-white/10 w-full max-w-sm rounded-[32px] shadow-2xl p-8 text-center animate-slide-down relative">
                        <div className="w-16 h-16 bg-red-500/10 text-red-500 border border-red-500/20 rounded-[20px] flex items-center justify-center mx-auto mb-5"><LogOut size={32} className="ml-1" /></div>
                        <h3 className="font-black text-white text-xl mb-2">Keluar Sesi?</h3>
                        <p className="text-gray-400 text-sm mb-8 leading-relaxed font-medium">Anda akan diarahkan kembali ke layar Login.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setLogoutModalOpen(false)} className="flex-1 py-4 bg-[#252530] text-gray-300 rounded-2xl font-bold hover:bg-white/10 transition-colors">Batal</button>
                            <button onClick={executeLogout} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-bold hover:bg-red-600 transition-colors shadow-lg shadow-red-500/20">Ya, Keluar</button>
                        </div>
                    </div>
                </div>
            )}
