        ...
        <div className="flex flex-col h-full w-full max-w-4xl mx-auto pb-24 relative px-4">
            ...
            <div className="flex gap-3 mb-8">
                <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="Cari bahan..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-white rounded-xl border border-gray-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#A7D189] transition-all" />
                </div>
                ...
                {filteredBahan.map((item) => (
                    <div key={item.id} className="relative group cursor-pointer">
                        ...
                        <button
                            ...
                            className="absolute bottom-3 right-3 p-2 bg-red-50 text-red-500 rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all shadow-sm hover:bg-red-100 hover:scale-110"
                        >
                            <Trash2 size={18} />
                        </button>
                    </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <div className="w-1/3">
                                    ...
                                    <select value={newBahan.satuan} onChange={(e) => setNewBahan({ ...newBahan, satuan: e.target.value })} className="w-full px-3 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#A7D189] transition-all">
                                        <option value="kg">kg</option><option value="gram">gram</option><option value="Liter">Liter</option><option value="Pcs">Pcs</option><option value="Ikat">Ikat</option>
                                    </select>
                                </div>
                            </div>
            ...
            {deleteModal.isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 text-center animate-slide-down relative">
                        ...
                        <h3 className="font-bold text-gray-800 text-xl mb-2">Buang Bahan?</h3>
                        <p className="text-gray-500 text-sm mb-6 leading-relaxed">
                            Apakah Anda yakin ingin menghapus <span className="font-bold text-gray-800">"{deleteModal.namaBahan}"</span> secara permanen?
                        </p>
                        <div className="flex gap-3">
                            <button onClick={() => setDeleteModal({ isOpen: false, idBahan: null, namaBahan: '' })} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition-colors">Batal</button>
                            <button onClick={executeDelete} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors shadow-md">Ya, Buang</button>
                        </div>
                    </div>
                </div>
            )}
